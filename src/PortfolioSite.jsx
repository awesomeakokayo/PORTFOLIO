...
import { ArrowRight, ArrowUpRight, Brain, Check, ExternalLink, Menu, Rocket, Smartphone, X } from "lucide-react";
...
const moreWork = [
  ["Southwest Flood Monitor", "Civic technology", "A community-focused warning system exploring earlier, clearer flood alerts."],
  ["AE-FUNAI Journal of Education", "Institutional platform", "A digital workflow for academic submission, review and publication."],
  ["Blancquake Foundation", "Non-profit", "A stronger digital presence for neuro-health and blood donation advocacy."],
  ["CCU Journal of Education", "Academic publishing", "An in-house publishing workflow built for a university environment."],
];

const services = [
  { icon: Smartphone, title: "Web & mobile products", text: "Customer-facing websites, SaaS products, dashboards and mobile apps — from the first screen to the backend behind it." },
  { icon: Brain, title: "AI-powered features", text: "Useful AI experiences integrated into real products: assistants, recommendations, analysis, automation and intelligent workflows." },
  { icon: Rocket, title: "Idea → working product", text: "For founders and teams with a concept that needs shaping, building, testing and shipping instead of staying in a slide deck." },
];
...
function ProjectVisual({ type }) {
  ...
  if (type === "habit") {
    const bars = [24, 42, 30, 55, 47, 68, 76];
    return (
      <div className="relative min-h-[260px] overflow-hidden rounded-[20px] bg-[#10100f] p-5 sm:min-h-[340px]">
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-white/10 bg-[#080808]/85 backdrop-blur-xl">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 sm:px-7 lg:h-[76px] lg:px-10 xl:px-16">
        ...
        <nav className="hidden items-center gap-8 text-[11px] uppercase tracking-[0.18em] md:flex" style={mono}>{links.map(([label, href]) => <a key={label} href={href} className="text-[#9c9b94] hover:text-white">{label}</a>)}</nav>
        <a href="#contact" className="hidden items-center gap-2 rounded-full bg-[#ff5c00] px-5 py-2.5 text-xs font-bold text-[#080808] md:inline-flex" style={mono}>Let’s talk <ArrowRight className="h-3.5 w-3.5" /></a>
